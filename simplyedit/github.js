  ...
  var Github = function (options) {
    ...
    function _request(method, path, data, cb, raw, sync) {
      ...
      if (options.token || (options.username && options.password)) {
        var authorization = options.token
          ? "token " + options.token
    ...
    Github.User = function () {
      ...
      this.userRepos = function (username, cb) {
        _requestAllPages(
          "/users/" + username + "/repos?type=all&per_page=1000&sort=updated",
          function (err, res) {
            cb(err, res);
          }
        );
      ...
      this.orgRepos = function (orgname, cb) {
        _requestAllPages(
          "/orgs/" +
            orgname +
            "/repos?type=all&&page_num=1000&sort=updated&direction=desc",
          function (err, res) {
            cb(err, res);
          }
        );
    ...
    Github.Repository = function (options) {
      ...
      this.listPulls = function (state, cb) {
        _request(
          "GET",
          repoPath + "/pulls" + (state ? "?state=" + state : ""),
          null,
          function (err, pulls) {
      ...
      this.getSha = function (branch, path, cb) {
        ...
        _request(
          "GET",
          repoPath + "/contents/" + path + (branch ? "?ref=" + branch : ""),
          null,
      ...
      this.postBlob = function (content, cb) {
        if (typeof content === "string") {
          content = { content: content, encoding: "utf-8" };
        } else {
          content = {
            content: btoa(
      ...
      this.commit = function (parent, tree, message, cb) {
        ...
        user.show(null, function (err, userData) {
          ...
          var data = {
            message: message,
            author: { name: options.user, email: userData.email },
            parents: [parent],
            tree: tree,
          };
          _request(
      ...
      this.branch = function (oldBranch, newBranch, cb) {
        ...
        this.getRef("heads/" + oldBranch, function (err, ref) {
          ...
          that.createRef({ ref: "refs/heads/" + newBranch, sha: ref }, cb);
        });
      };
      ...
      this.remove = function (branch, path, cb) {
        that.getSha(branch, path, function (err, sha) {
          ...
          _request(
            ...
            { message: path + " is removed", sha: sha, branch: branch },
            cb
          );
        });
      };
      this.delete = function (branch, path, cb) {
        that.getSha(branch, path, function (err, sha) {
          ...
          var params = { message: "Deleted " + path, sha: sha };
      ...
      this.write = function (branch, path, content, message, cb) {
        /*that.getSha(branch, encodeURI(path), function (err, sha) {
          ...
          _request(
            "PUT",
            repoPath + "/contents/" + encodeURI(path),
            {
              message: message,
        ...
        that.getSha(branch, encodeURI(path), function(err, sha) {
          // SE il file non viene trovato (err è vero), lo SHA è indefinito.
          // Questo è il caso della CREAZIONE di un nuovo file.
          // Procediamo comunque, ma senza passare lo SHA.
          if (err) {
              sha = undefined;
          }
          ...
          // Se lo SHA esiste (cioè stiamo AGGIORNANDO un file), lo aggiungiamo.
          if (sha) {
              data.sha = sha;
          }

          _request("PUT", repoPath + "/contents/" + encodeURI(path), data, function(err, res) {
              if (err) {
                  return cb(err);
              }
              cb(null, res);
